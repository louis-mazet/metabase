...
const initSqlParser = function (parser) {
  ...
  parser.suggestValueExpressionKeywords = function (valueExpression, extras) {
    const expressionKeywords = parser.getValueExpressionKeywords(
      valueExpression,
      extras,
    );
  ...
  parser.commitLocations = function () {
    ...
    if (parser.yy.locations.length > 0) {
      parser.yy.allLocations = parser.yy.allLocations.concat(
        parser.yy.locations,
      );
      parser.yy.locations = [];
    }
      ...
      if (
        ...
        for (
          ...
          if (
            ...
            typeof cteIndex[table.identifierChain[0].name.toLowerCase()] !==
              "undefined"
          ) {
      ...
      if (
        ...
        if (typeof parser.yy.result.suggestColRefKeywords !== "undefined") {
          Object.keys(parser.yy.result.suggestColRefKeywords).forEach(type => {
            parser.yy.result.suggestKeywords =
              parser.yy.result.suggestKeywords.concat(
  ...
  parser.expandIdentifierChain = function (options) {
    ...
    let tablePrimaries =
      options.tablePrimaries || parser.yy.latestTablePrimaries;
    ...
    if (!anyOwner) {
      tablePrimaries = filterTablePrimariesForOwner(
        tablePrimaries,
        wrapper.owner,
      );
    }
    ...
    if (
      identifierChain.length > 0 &&
      identifierChain[identifierChain.length - 1].asterisk
    ) {
      const tables = [];
      tablePrimaries.forEach(tablePrimary => {
        if (identifierChain.length > 1 && !tablePrimary.subQueryAlias) {
          ...
          tables.push({
            identifierChain: [{ subQuery: tablePrimary.subQueryAlias }],
          });
        } else {
          addCleanTablePrimary(tables, tablePrimary);
        }
    ...
    if (foundPrimary) {
      if (isColumnWrapper) {
        ...
        if (foundPrimary.subQueryAlias) {
          ...
          identifierChain =
            foundPrimary.identifierChain.concat(identifierChain);
        }
        if (wrapper.tables) {
  ...
  const linkTablePrimaries = function () {
    if (
      ...
      typeof parser.yy.latestTablePrimaries === "undefined"
    ) {
      return;
    }
    ...
    SIMPLE_TABLE_REF_SUGGESTIONS.forEach(suggestionType => {
      if (
        ...
        parser.yy.result[suggestionType].tablePrimaries.forEach(
          tablePrimary => {
    ...
    if (
      ...
      if (
        typeof parser.yy.result.suggestColumns.identifierChain ===
          "undefined" ||
        parser.yy.result.suggestColumns.identifierChain.length === 0
      ) {
    ...
    if (
      typeof parser.yy.result.colRef !== "undefined" &&
      !parser.yy.result.colRef.linked
    ) {
      parser.expandIdentifierChain({ wrapper: parser.yy.result.colRef });

      const primaries = filterTablePrimariesForOwner(
        parser.yy.latestTablePrimaries,
      );
      if (
        primaries.length === 0 ||
        (primaries.length > 1 &&
          parser.yy.result.colRef.identifierChain.length === 1)
      ) {
  ...
  parser.checkForSelectListKeywords = function (selectList) {
    ...
    const valueExpressionKeywords = parser.getValueExpressionKeywords(
    ...
    if (valueExpressionKeywords.suggestColRefKeywords) {
      parser.suggestColRefKeywords(
        valueExpressionKeywords.suggestColRefKeywords,
      );
  ...
  parser.addColRefToVariableIfExists = function (left, right) {
    if (
      ...
      if (/\${[^}]*}/.test(left.columnReference[0].name)) {
        // left is variable
        addColRefToVariableLocation(
          left.columnReference[0].name,
          right.columnReference,
        );
      } else if (/\${[^}]*}/.test(right.columnReference[0].name)) {
        // right is variable
        addColRefToVariableLocation(
          right.columnReference[0].name,
          left.columnReference,
        );
      }
    }
  ...
  parser.handleQuotedValueWithCursor = function (
    lexer,
    yytext,
    yylloc,
    quoteChar,
    ...
    if (yytext.indexOf("\u2020") !== -1 || yytext.indexOf("\u2021") !== -1) {
      ...
      const remainingQuotes = (
        lexer.upcomingInput().match(new RegExp(quoteChar, "g")) || []
      ).length;
  ...
  parser.parseSql = function (beforeCursor, afterCursor, debug) {
    ...
    if (beforeCursor.length > REASONABLE_SURROUNDING_LENGTH) {
      ...
      beforeCursor = lastReasonableChunk.substring(
        lastReasonableChunk.indexOf(";") + 1,
      );
    }

    if (afterCursor.length > REASONABLE_SURROUNDING_LENGTH) {
      if (
        afterCursor.length - afterCursor.indexOf(";") >
        REASONABLE_SURROUNDING_LENGTH
      ) {
    ...
    parser.yy.partialLengths = parser.identifyPartials(
      beforeCursor,
      afterCursor,
    );
    ...
    if (
      ...
      typeof result.error.expected !== "undefined"
    ) {
      ...
      if (location.type === "statement") {
        if (lastStatementLocation === null) {
          ...
          location.location.first_line =
            lastStatementLocation.location.last_line;
          location.location.first_column =
            lastStatementLocation.location.last_column + 1;
        }
