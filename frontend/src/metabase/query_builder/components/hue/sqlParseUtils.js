...
export const equalIgnoreCase = (a, b) =>
...
export const initSharedAutocomplete = parser => {
  ...
  parser.addClauseLocation = (
    ...
    if (isCursor) {
      if (
        ...
        parser.yy.partialLengths.right === 0
      ) {
        location = {
          type: type,
          missing: true,
  ...
  parser.addColumnAliasLocation = (location, alias, parentLocation) => {
    ...
    if (
      ...
      if (
        closestColumn.location.first_line ===
          aliasLocation.parentLocation.first_line &&
        closestColumn.location.last_line ===
          aliasLocation.parentLocation.last_line &&
        closestColumn.location.first_column ===
          aliasLocation.parentLocation.first_column &&
        closestColumn.location.last_column ===
          aliasLocation.parentLocation.last_column
      ) {
  ...
  parser.applyTypeToSuggestions = details => {
    ...
    if (
      parser.yy.result.suggestFunctions &&
      !parser.yy.result.suggestFunctions.types
    ) {
      parser.applyTypes(parser.yy.result.suggestFunctions, details);
    }
    if (
      parser.yy.result.suggestColumns &&
      !parser.yy.result.suggestColumns.types
    ) {
  ...
  parser.valueExpressionSuggest = (oppositeValueExpression, operator) => {
    ...
    if (
      oppositeValueExpression &&
      oppositeValueExpression.types[0] === "NUMBER"
    ) {
